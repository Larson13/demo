...
;(function ($) {    // 此处将$作为匿名函数的形参
    $.extend({
        ...
        dataTable: function (selector, config) {
            ...
            common_data.ajax.url = location.origin + common_data.ajax.url;
            //返回一个数据表格对象，进行一些动态操作
            return {
                    ...
                    ajax: function (data, callback, settings) {//ajax配置为function,手动调用异步查询
                        $.ajax({
                            ...
                            success: function (res) {
                                ...
                                if (common_data.bPaginate) {
                                    returnData.data = res.data.list == null ? [] : res.data.list ;
                                    common_data.resultData=res.data.list == null ? [] : res.data.list;
                                    returnData.recordsTotal = res.data.totalNums;
                                    returnData.recordsFiltered = res.data.totalNums;//后台不实现过滤功能，每次查询均视作全部结果
                                }else{
                                    returnData.data = res.data;
                                    common_data.resultData=res.data;
                                }
                                callback(returnData);
                            },
